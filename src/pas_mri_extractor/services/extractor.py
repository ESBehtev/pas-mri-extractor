"""Главная логика извлечения признаков PAS из MRI-текста.
Зачем нужен:
- собрать extraction prompt без изменения clinical schema;
- вызвать LLM и извлечь JSON из raw output;
- валидировать результат через Pydantic перед scoring.
"""
...
def parse_json_with_retry(
    ...
    retry_raw_output = client.generate(
        prompt,
